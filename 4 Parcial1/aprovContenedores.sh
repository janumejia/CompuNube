#!/bin/bash
echo "Creacion contenedor haproxyC"
sudo lxc init ubuntu:20.04 haproxyC --target haproxy < /dev/null

echo "Creacion contenedor web1C"
sudo lxc init ubuntu:20.04 web1C --target web1 < /dev/null

echo "Creacion contenedor web2C"
sudo lxc init ubuntu:20.04 web2C --target web2 < /dev/null

echo "Encender contenedores"
sudo lxc start haproxyC web1C web2C

echo "actualizacion contenedores"
sudo lxc exec haproxyC -- sudo apt update && apt upgrade -y
sudo lxc exec web1C -- sudo apt update && apt upgrade -y
sudo lxc exec web2C -- sudo apt update && apt upgrade -y
...
backend web-backend
   ...
   server web1 240.3.0.189:80 check
   server web2 240.4.0.22:80 check

   #Servidores web de respaldo
   option allbackups
   server web3 240.3.0.137:80 check backup
   server web4 240.4.0.120:80 check backup
...
backend web-backend-full
   ...
   server web1 240.3.0.189:80 check
   server web2 240.4.0.22:80 check
   server web3 240.3.0.137:80 check
   server web4 240.4.0.120:80 check

frontend http
  bind *:80
  #Sí hay mas de 15 o mas conexion, cambia al backend donde funcionan los 4 con roundrobin
